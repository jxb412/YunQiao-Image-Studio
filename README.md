...
```text
https://github.com/jxb412/YunQiao-Image-Studio/releases/tag/v0.1.14
```

客户端文件：

- Windows x64 便携版：`YunQiao-Image-Studio-0.1.14-win-x64-portable.exe`
...
- 程序代理：支持 HTTP/HTTPS 代理，便于网络受限地区访问 API、更新服务和远程图片。
- 本机安全存储：API Key 和云存储密钥只保存在本机安全存储中。
- 跨平台配置：同一套代码支持 Windows 和 macOS 构建。

## v0.1.14 更新重点

- 设置页新增程序代理配置，支持 `http://127.0.0.1:7890`、`https://host:port` 和带账号密码的 HTTP/HTTPS 代理。
- 生图、修图、API 测试、更新检查和远程图片下载统一走主进程代理封装。
- 代理测试会显示 HTTP 状态、耗时、直连/代理模式和返回摘要，方便排查网络连通性。
...
- 程序代理：可启用 HTTP/HTTPS 代理，例如 `http://127.0.0.1:7890`；保存后会用于 API、生图、修图、更新检查和远程图片下载。
