...
```text
https://github.com/jxb412/YunQiao-Image-Studio/releases/tag/v0.1.11
```

客户端文件：

- Windows x64 便携版：`YunQiao-Image-Studio-0.1.11-win-x64-portable.exe`
...
- 自动更新检查：启动后读取网站更新清单并对比当前版本，GitHub 最新 Release 作为备用节点。
- 跨平台配置：同一套代码支持 Windows 和 macOS 构建。

## v0.1.11 更新重点

- 新增启动自动检查更新：程序启动后自动读取 `https://down.haowucm.cn/yunqiao/latest.json`，对比当前版本并提示下载。
- 手动检查更新也改为优先使用网站更新节点；如果网站节点不可用或 GitHub 版本更新，会自动退回 GitHub 最新 Release。
- 更新清单会按当前系统选择对应客户端：Windows x64、macOS Intel x64、macOS Apple Silicon arm64。
- GitHub Release workflow 会生成带文件大小和 SHA256 的 `latest.json`，并可通过 FTP 上传到下载站镜像。
- 下载站主节点使用 `https://down.haowucm.cn/yunqiao/releases/v版本号/文件名`，更新失败时仍可使用 GitHub Release 备用下载。
- 保留 v0.1.10 的年轻化行业模板扩充：人物图像、二次元头像、潮流社交、虚拟IP、自媒体、漫画动漫、游戏美术、潮玩玩具、个人 IP 等。
...
- 同时生成 `latest.json` 更新清单；配置 FTP Secret 后会自动上传到网站下载镜像。
...
清单内会给出三端客户端的主下载地址、GitHub 备用地址、文件大小和 SHA256。客户端会优先使用网站镜像；如果网站节点不可用，或 GitHub Release 版本更新，则使用 GitHub 备用节点。默认网站镜像路径：
...
FTP 上传账号密码不要写入代码。自动上传时使用 GitHub Secrets：
...
默认公开下载域名为 `https://down.haowucm.cn/yunqiao`，默认 FTP 远程目录为 `yunqiao`。
