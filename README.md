...
```text
https://github.com/jxb412/YunQiao-Image-Studio/releases/tag/v0.1.15
```

客户端文件：

- Windows x64 便携版：`YunQiao-Image-Studio-0.1.15-win-x64-portable.exe`
...
- 程序代理：支持跟随 Windows/macOS 系统代理，也支持手动 HTTP/HTTPS 代理和直连模式。
- 本机安全存储：API Key 和云存储密钥只保存在本机安全存储中。
- 跨平台配置：同一套代码支持 Windows 和 macOS 构建。

## v0.1.15 更新重点

- 程序代理升级为三种模式：跟随系统代理、手动代理、直连。
- 默认跟随 Windows/macOS 系统网络代理；支持系统手动代理和 PAC 自动代理解析出的 HTTP/HTTPS 代理。
...
- 程序代理：默认跟随系统代理，也可改为手动 HTTP/HTTPS 代理或直连；保存后会用于 API、生图、修图、更新检查和远程图片下载。
